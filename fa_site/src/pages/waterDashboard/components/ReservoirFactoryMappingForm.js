...
const ReservoirFactoryMappingForm = () => {
  ...
  const factoryAggregationRows = useMemo(() => {
    ...
    const groupedRows = Array.from(bucket.values())
      ...
      // 按北往南排序：取廠區對應水庫的最大緯度（最北的），按降序排列
      .sort((a, b) => {
        const aMaxLat = Math.max(...a.reservoirs.map((name) => RESERVOIR_LATITUDE_MAP[name] ?? 0));
        const bMaxLat = Math.max(...b.reservoirs.map((name) => RESERVOIR_LATITUDE_MAP[name] ?? 0));
        return bMaxLat - aMaxLat; // 降序：北到南
      });
